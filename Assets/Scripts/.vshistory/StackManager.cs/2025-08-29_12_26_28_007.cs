...
public class StackManager : MonoBehaviour
{
        ...
        if (!gameStarted && !blockIsDropping && !settingsManager.IsSettingsOpen() && !powerupsManager.IsPowerupsOpen() && Input.GetMouseButtonDown(0))
        {
            ...
            foreach (var item in selected)
            {
                switch (item.ID)
                {
                    ...
                    case "EXTRA_SLOWTIME_03":
                        hasSlowTime = true;
                        break;

                    case "EXTRA_PERFECTMAGNET_04":
                        hasPerfectMagnet = true;
                        perfectStackThreshold = 0.25f; // wider tolerance
                        break;
            ...
            uiManager.AnimatePowerupBadgesOut();
    ...
    private void SpawnFirstBlock()
    {
        ...
        lastBlock.GetComponent<Renderer>().material = baseMaterial;
    ...
    private void SpawnNextBlock()
    {
        ...
        //newBlock.GetComponent<Renderer>().material
        ...
        if (hasSlowTime)
        {
            float t = Mathf.Clamp01(score / slowTimeRecoveryScore);
            float speedMultiplier = Mathf.Lerp(slowTimeFactor, 1f, t);
    ...
    private void PlaceBlock()
    {
        ...
        if (overlap <= 0f)
        {
            ...
            {
                extraLivesRemaining--;
                hasExtraLife = false; // consume it
                Destroy(rbLoss); // stop the death fall
                gameOver = false;
                currentBlock.DOMove(Vector3.zero, 0.4f);
                currentBlock.DOScale(new Vector3(5, 1, 5), 0.4f);
                blockIsDropping = false;
                SpawnNextBlock();
                return; // don’t trigger game over UI
            }
        ...
        if (Mathf.Abs(delta) <= perfectStackThreshold)
        {
            ...
            perfectStreak++;
            int coinsToAdd = perfectStreak >= 3 ? 3 : 1;
            if (hasCoinDoubler) coinsToAdd *= 2;
    ...
    public void ResetGame()
    {
        ...
        uiManager.FadeToBlack(() =>
        {
            ...
            uiManager.ResetPowerupBadges();
            uiManager.ResetPowerupColors();
            powerupsManager.ResetSelectedPowerUps();
