...
public class StackManager : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        if (!gameStarted && !blockIsDropping && !settingsManager.IsSettingsOpen() && !powerupsManager.IsPowerupsOpen() && Input.GetMouseButtonDown(0))
        {
            ...
            powerupsManager.RemoveRequiredPowerup();
    ...
    private void SpawnNextBlock()
    {
        ...
        if (hasSlowTime)
        {
            float t = Mathf.Clamp01((score - (hasHigherStart ? higherStartScore : 0)) / slowTimeRecoveryScore);
    ...
    private void TriggerGameOverEffects()
    {
        ...
        if (hasCoinDoubler)
            hasCoinDoubler = false;
        if (hasSlowTime)
            hasSlowTime = false;
        if (hasPerfectMagnet)
        {
            hasPerfectMagnet = false;
            perfectStackThreshold = originalPerfectThreshold;
        }
