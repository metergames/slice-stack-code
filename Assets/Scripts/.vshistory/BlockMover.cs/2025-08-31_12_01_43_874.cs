...
public class BlockMover : MonoBehaviour
{
    ...
    private float speedMultiplier = 1f; // default speed
    ...
    public void StartMovement()
    {
        ...
        float adjustedDuration = moveDuration / speedMultiplier;

        moveTween = transform.DOMove(endPos, adjustedDuration).SetEase(Ease.InOutSine).SetLoops(-1, LoopType.Yoyo);
    }
    ...
    {
        speedMultiplier = Mathf.Max(0.01f, multiplier); // avoid divide by zero

        // If currently moving, restart movement at new speed
