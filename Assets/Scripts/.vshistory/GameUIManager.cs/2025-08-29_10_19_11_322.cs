...
public class GameUIManager : MonoBehaviour
{
    ...
    public void AnimatePowerupBadgesOut(System.Action onComplete = null)
    {
        ...
        {
            int index = i;
            RectTransform child = powerupBadgesParent.GetChild(index).GetComponent<RectTransform>();

            child.DOKill();

            child.DOAnchorPosX(child.anchoredPosition.x + moveDistance, duration)
                 .SetEase(Ease.InBack)
    ...
    public void ResetPowerupBadges()
    {
        ...
        for (int i = 0; i < childCount; i++)
        {
            RectTransform child = powerupBadgesParent.GetChild(i).GetComponent<RectTransform>();

            child.DOKill();
            child.anchoredPosition = new Vector2(-115, child.anchoredPosition.y);
        }

        powerupsManager.powerUps.Clear();
    }

    public void ResetPowerupColors()
    {
        Debug.Log(powerupsManager.powerUps.Count);
        foreach (var p in powerupsManager.powerUps)
        {
            p.isSelected = false; // clear selection state
            p.background.color = powerupsManager.unselectedColor; // reset to yellow
        }
    }
}
