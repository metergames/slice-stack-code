...
public class GameUIManager : MonoBehaviour
{
    ...
    public void ShowStartUI()
    {
        ...
        if (powerupBadgesParent != null)
            powerupBadgesOriginalPos = powerupBadgesParent.anchoredPosition;
    ...
    public void AnimatePowerupBadgesOut(System.Action onComplete = null)
    {
        ...
        // Step 1: Nudge left, then right off-screen
        Sequence seq = DOTween.Sequence();
        seq.Append(powerupBadgesParent.DOAnchorPosX(powerupBadgesOriginalPos.x - 50f, duration * 0.35f)
                                       .SetEase(Ease.InOutQuad));
        seq.Append(powerupBadgesParent.DOAnchorPosX(powerupBadgesOriginalPos.x + 200f, duration)
                                       .SetEase(Ease.InBack));

        // Step 2: When off-screen, disable children
        seq.OnComplete(() =>
        {
            foreach (Transform child in powerupBadgesParent)
    ...
    public void ResetPowerupBadges()
    {
        ...
        powerupBadgesParent.anchoredPosition = powerupBadgesOriginalPos;
